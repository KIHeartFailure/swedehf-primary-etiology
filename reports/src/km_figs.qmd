...
kmfunc <- function(efcat, time, event, eventname) {
  ...
  p <- ggplot(plot_data, aes(x = time / 365, y = estimate, color = strata)) +
    ...
    annotate("text", x = 0, y = c(1, .9, .8, .7, .6) + addon, label = c("", levs), hjust = 0, size = 6, colour = c("black", global_cols[1:4])) +
    annotate("text", x = 3.5, y = c(1, .9, .8, .7, .6) + addon, label = cox_crude, hjust = 0.5, size = 6, colour = c("black", global_cols[1:4])) +
    annotate("text", x = 7, y = c(1, .9, .8, .7, .6) + addon, label = cox_adj, hjust = 0.5, size = 6, colour = c("black", global_cols[1:4]))
