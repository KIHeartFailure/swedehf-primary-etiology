...
vars <- bind_rows(
  tibble(var = c("shf_primaryetiology_cat", tabvars)),
  outvars %>%
...
ihdspec <- str_split(str_trim(outcommeta %>% filter(Variable %in% c("sos_com_pci", "sos_com_cabg", "sos_com_ihd")) %>% pull(Code)), " ", simplify = T)
...
footnotetext <- c(
  ...
  "Medications derived from prescriptions within 4 months prior to index up untill 14 days after index.",
...
vars_xls <- vars %>%
  ...
  add_row(Variable = paste0("b ", footnotetext[3])) %>%
...
default_kable(vars,
  ...
  footnote(
    ...
    general_title = "",
    general = c(
      "Abbreviations: SwedeHF, The Swedish Heart Failure Registry; NPR, The National Patient Register; CDR, The Cause of Death Register; NPDR, National Prescribed Drug Register; LISA, Longitudinal integrated database for health insurance and labour market studies; RTP, Register of the Total Population"
    ),
