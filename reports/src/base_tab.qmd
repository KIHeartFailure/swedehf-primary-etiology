...
basetabfunc <- function(data, x = "shf_primaryetiology_cat", novars = NULL) {
  tab <- data %>%
    count(!!sym(x), .drop = F) %>%
    ...
    select(var, level, pmis, levels(rsdata %>% pull(!!sym(x))), p)

  tabvars2 <- setdiff(tabvars, novars)
  tabtmp <- sapply(tabvars2,
    FUN = basetabfunc_inner, data = data,
    stratavar = x, catdig = catdig, contdig = contdig
  )
  ...
  cn <- c("Variable", c("Missing (%)", levels(data %>% pull(!!sym(x))), "p-value"))
  ...
  default_kable(tab,
    ...
    add_header_above(c(" " = 1, " " = 1, "Primary etiology" = ncol, " " = 1)) %>%
    footnote(
...
basetabfunc(data = rsdata %>% filter(shf_sex == "Female"), novar = "shf_sex")
```
...
basetabfunc(data = rsdata %>% filter(shf_sex == "Male"), novar = "shf_sex")
```

\blandscape

```{r}
#| label: tbl-base-addetiology
#| cache: true
#| cache.comments: false
#| dependson: basetabfunc
#| tbl-cap: "Baseline characteristics - additonal etiologies"
#| tbl-pos: "H"

basetabfunc(data = rsdata, x = "shf_primaryetiology2")
```
