...
basetabfunc <- function(data, novars = NULL) {
  tab <- data %>%
    count(shf_primaryetiology_cat, .drop = F) %>%
    mutate(n = paste0(n, " (", fn(n / sum(n) * 100, catdig), ")")) %>%
    pivot_wider(names_from = shf_primaryetiology_cat, values_from = n) %>%
    mutate(
      var = "N (%)",
      level = NA,
      p = NA,
    ...
    select(var, level, pmis, levels(rsdata %>% pull(shf_primaryetiology_cat)), p)

  tabvars2 <- setdiff(tabvars, novars)
  tabtmp <- sapply(tabvars2,
    FUN = basetabfunc_inner, data = data,
    stratavar = "shf_primaryetiology_cat", catdig = catdig, contdig = contdig
  )
  ...
  cn <- c("Variable", c("Missing (%)", levels(data %>% pull(shf_primaryetiology_cat)), "p-value"))
  ...
  default_kable(tab,
    ...
    add_header_above(c(" " = 1, " " = 1, "Primary etiology" = 4, " " = 1)) %>%
    footnote(
